            ...
            public void onClick(View v) {
                client.getLastLocation().addOnSuccessListener(MainActivity.this, new OnSuccessListener<Location>() {
                    ...
                    public void onSuccess(Location location) {
                        if(location != null) {
                            Double lng = location.getLongitude();//(double) Math.round(location.getLongitude() * 100) / 100;
                            Double lat = location.getLatitude();//double) Math.round(location.getLatitude() * 100) / 100;
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    for(Address add: addresses){
                        Double lng = add.getLongitude();//double) Math.round(add.getLongitude() * 100) / 100;
                        Double lat = add.getLatitude();//double) Math.round(add.getLatitude() * 100) / 100;
